...
Compte tenu de la taille (`nrow(accidents)` observations) de nos données et des
...
On introduit les fonctions suivantes pour mesurer l'erreur de précision de nos
...
```{r, include=FALSE}
load("~/envir_modeles_2021-03-16.rda")
```
...
On peut également représenter graphiquement ces résultats
...
On retient la méthode de gradient boosting comme la meilleure méthode pour ce
jeu de données.

On se propose donc d'ajuster 
