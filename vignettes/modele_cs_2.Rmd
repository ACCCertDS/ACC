...
accidents_M <- accidents_M %>%
    mutate(blessures = case_when(
      grav == "Indemne"~ 0,
      TRUE ~ 1) ) %>%
    select(-Num_Acc,-grav) 
...
dummy_A <- dummyVars(" ~ .", data=don_A %>% select(-blessures))
don_X_A <- data.frame(predict(dummy_A, newdata = don_A %>% select(-blessures))) 

don_Y_A <- don_A$blessures

# échantillon test
dummy_T <- dummyVars(" ~ .", data=don_T %>% select(-blessures))
don_X_T <- data.frame(predict(dummy_T, newdata = don_T %>% select(-blessures))) 
```
...
# estimation du temps d'éxécution
time6 <- Sys.time()
paste("temps execution de l'estimateur ridge : ", Tdiff= difftime(time6, time5))
```
...
# estimation du temps d'éxécution
time8 <- Sys.time()
paste("temps execution de l'estimateur elastic net : ", Tdiff= difftime(time8, time7))
```
...
On crée la variable controle pour utiliser la fonction train() de caret dans les prochains modèles
```{r}
...
# estimation du temps d'éxécution
time10 <- Sys.time()
paste("temps execution de la méthode CART : ", Tdiff= difftime(time10, time9))
```
...
# estimation du temps d'éxécution
time12 <- Sys.time()
paste("temps execution de la méthode AdaBoost : ", Tdiff= difftime(time12, time11))
```
...
# estimation du temps d'éxécution
time14 <- Sys.time()
paste("temps execution de la méthode LogitBoost : ", Tdiff= difftime(time14, time13))
```
...
paste("erreur minimale : ",error_min)

error_max <- max(criteres_erreur_classif_plot$Error)
paste("erreur maximale : ",error_max)
...
```{r}
don_T_prev <- don_T[, grep("bles_",colnames(don_T))]
...
don_T$bles_prev_proba_agreg <- apply(don_T_prev,1,max)
...
don_T$bles_prev_proba_agreg <- apply(don_T_prev,1,min)
...
### attention pour le moment pas de résultat pour ces modèles (temps d'éxécution trop long donc arrêt manuel)...
ne pas exécuter le code ci-dessous


On crée la variable controle pour utiliser la fonction train() de caret dans les prochains modèles
```{r}
controle2 <- trainControl(method="cv",
                         number=5,             # 5 blocs 
                         classProbs=FALSE,
                         allowParallel=TRUE)
```
...
SVM (Noyau polynomial de degré 2) - Prédiction & Evaluation du modèle
```{r}
...
SVM (Noyau radial) - Prédiction & Evaluation du modèle
```{r}
