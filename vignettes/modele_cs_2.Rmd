...
accidents_M <- accidents2 %>%
    mutate(accidents, blessures = case_when(
      grav == "Indemne"~ 0,
      TRUE ~ 1) ) %>%
    select(-c(Num_Acc, num_veh, dep, lat, long, dtm_num, heure, age, pop_moy, grav)) 
...
```{r}
accidents_A <- accidents_M %>%
  filter(year(date_acc) %in% c(2015, 2016, 2017,2018)) %>%
  select(-date_acc)

accidents_A <- accidents_A %>% na.omit()

set.seed(8)
ind <- sample(nrow(accidents_A), 30000)
don_A <- accidents_A[ind,]
...
```{r}
accidents_T <- accidents_A[-ind,]

accidents_T <- accidents_T %>% na.omit()

set.seed(20)
ind <- sample(nrow(accidents_T), 10000)
don_T <- accidents_T[ind,]
