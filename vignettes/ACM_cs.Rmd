...
Etape 1 : on fait un Mclust avec 6 à 10 classes
```{r,eval=FALSE}
mod_classif <- Mclust(base, G = 1:10)
mod_classif$BIC
```
les meilleurs résultats sont pour les modèles EEV
...
```
on retient 6 classes plutôt que 4 parce que ça minimise le BIC, et 6 plutôt que 10 car mdoèle plus parcimonieux

```{r}
mod_classif <- Mclust(base, G = 6L, modelNames = "EEV")
summary(mod_classif) 
```
...
lapply(res_mclust$category, function(.x) head(as.data.frame(.x)))
```
...
lapply(res_mclust2$category, function(.x) head(as.data.frame(.x)))
```
Classe 1 (81 193)
...
```{r}
mod_kmodes <- kmodes(as.matrix(base), 6) # on prend 9 classes comme ce que la classification a trouvé
```
...
```{r}
# ajout des classes dans la base
base_comp2 <- cbind.data.frame(base,classe_kmodes=as.factor(mod_kmodes$cluster))
```

## Quelles modalités sont le plus liées aux modalité de la gravité des blessures ?
```{r}
res_kmodes <- catdes(base_comp2, num.var = 2)  # on caractérise la variable "grav" (2e colonne dans la base)
#res_kmodes$category
lapply(res_kmodes$category, function(.x) head(as.data.frame(.x),n=10))
```
